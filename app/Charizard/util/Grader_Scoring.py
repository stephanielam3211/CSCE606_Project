...
def compute_grader_matching_weight(student: Student, course: Course):
    weight = 0
    if Scoring.is_instructor_and_ta_same(student, course):
        return Weights.NINFINITY

    # TODO: Check that this logic is required for Grader
...
def compute_grader_weight_matrix(students: List[Student], courses: List[Course]) -> List[List[int]]:
    return [[compute_grader_matching_weight(student, course) for course in courses] for student in students]